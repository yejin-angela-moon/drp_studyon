...
class StudyLocationViewModel: ObservableObject {
        ...
        db.collection("studyLocations").addSnapshotListener { (querySnapshot, error) in
            ...
            self.studyLocations = documents.compactMap { (queryDocumentSnapshot) -> StudyLocation? in
                ...
                let hours = hoursData.compactMapValues { hourData -> OpeningHours? in
                    guard let opening = hourData["opening"],
                        let closing = hourData["closing"] else {
                        return nil
                    }
    ...
    //                                 (open: $0["opening"] ?? "Closed", close: $0["closing"] ?? "Closed")
    //                             }
    //                 let envFactor = data["envFactor"] as? EnvFactor ?? nil
    //                 let dynamicData = envFactor.dynamicData as? [String: Double] ?? [:]
    //                 let staticData = envFactor.staticData as? [String: Double] ?? [:]
    //                 let atmosphere = envFactor.atmosphere as? [String] ?? []
